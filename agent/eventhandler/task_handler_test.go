...
// TestQueueEventProcessing tests the queue-based event processing system in TaskHandler
// It verifies that events are properly queued and processed in the correct order
func TestQueueEventProcessing(t *testing.T) {
        ctrl := gomock.NewController(t)
        defer ctrl.Finish()
        client := mock_ecs.NewMockECSClient(ctrl)
        ...
        // Create multiple tasks with multiple events
        taskARN1 := "taskARN1"
        taskARN2 := "taskARN2"
        taskARN3 := "taskARN3"
        ...
        var wg sync.WaitGroup
        wg.Add(3) // Expecting 3 task state changes to be submitted

        // We expect the events to be processed in the order they are added
        // for each task, but tasks can be processed concurrently
        client.EXPECT().SubmitTaskStateChange(gomock.Any()).Do(func(change ecs.TaskStateChange) {
                assert.Equal(t, taskARN1, change.TaskARN)
                assert.Equal(t, 2, len(change.Containers))
                wg.Done()
        })

        client.EXPECT().SubmitTaskStateChange(gomock.Any()).Do(func(change ecs.TaskStateChange) {
                assert.Equal(t, taskARN2, change.TaskARN)
                assert.Equal(t, 1, len(change.Containers))
                wg.Done()
        })

        client.EXPECT().SubmitTaskStateChange(gomock.Any()).Do(func(change ecs.TaskStateChange) {
                assert.Equal(t, taskARN3, change.TaskARN)
                assert.Equal(t, 1, len(change.ManagedAgents))
                wg.Done()
        })
        ...
        taskEvent1 := api.TaskStateChange{
                TaskARN: taskARN1, 
                Status: apitaskstatus.TaskRunning, 
                Task: &apitask.Task{},
        }
        ...
        contEvent3 := api.ContainerStateChange{
                TaskArn: taskARN2, 
                ContainerName: "container3", 
                Status: apicontainerstatus.ContainerRunning, 
                Container: &apicontainer.Container{},
        }
        taskEvent2 := api.TaskStateChange{
                TaskARN: taskARN2, 
                Status: apitaskstatus.TaskRunning, 
                Task: &apitask.Task{},
        }

        // Add events for task 3 with managed agent
        maEvent := api.ManagedAgentStateChange{
                TaskArn: taskARN3, 
                Name: "ExecAgent", 
                Status: apicontainerstatus.ManagedAgentRunning,
                Container: &apicontainer.Container{},
        }
        taskEvent3 := api.TaskStateChange{
                TaskARN: taskARN3, 
                Status: apitaskstatus.TaskRunning, 
                Task: &apitask.Task{},
        }

        // Add all events to the handler
        // Mix the order to ensure the handler correctly associates events with their tasks
        handler.AddStateChangeEvent(contEvent1, client)
        handler.AddStateChangeEvent(maEvent, client)
        handler.AddStateChangeEvent(contEvent2, client)
        handler.AddStateChangeEvent(contEvent3, client)
        
        // Add task events which should trigger the submission of the container events
        handler.AddStateChangeEvent(taskEvent1, client)
        handler.AddStateChangeEvent(taskEvent2, client)
        handler.AddStateChangeEvent(taskEvent3, client)
        ...
        // Verify that all events have been processed and maps are empty
        assert.Equal(t, 0, len(handler.tasksToContainerStates))
        assert.Equal(t, 0, len(handler.tasksToManagedAgentStates))
